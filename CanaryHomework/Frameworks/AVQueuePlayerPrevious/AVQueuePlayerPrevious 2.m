...
#define kReachedEndOfPlaylistNotification @"ReachedEndOfPlaylistNotification" // probably best if it's inside of the global constants file instead
#define kReachedEndOfClipNotification @"ReachedEndOfClipNotification"
...
{
    BOOL didLoop = [self shouldLoopPlayer];
    if (!didLoop) // only if we don't loop, update the index, otherwise it's already reset
    {
        ...
        if (nowPlayingIndex < [_itemsForPlayer count] - 1)
        {
            nowPlayingIndex++;
            @try {
                [[NSNotificationCenter defaultCenter]postNotificationName:kReachedEndOfClipNotification object:[NSNumber numberWithInt:nowPlayingIndex]];
            }
        ...
        else if (nowPlayingIndex == self.itemsForPlayer.count - 1) // reached end of the playlist
        {
            DLog(@"Reached end of playlist, firing notification");
            [[NSNotificationCenter defaultCenter]postNotificationName:kReachedEndOfPlaylistNotification object:nil];
        }
    }
}
...
-(void)playPreviousItem
{
    ...
    {
        [self resetPlaylistFromIndex:nowPlayingIndex - 1];
    }
}


-(BOOL)shouldLoopPlayer
{
    BOOL hasLooped = NO;
    
    if (self.isLooping)
    {
        if (nowPlayingIndex == self.itemsForPlayer.count - 1) // reached end of the playlist
        {
            [self resetPlaylistFromIndex:0];
            hasLooped = YES;
        }
    }
...
-(void)resetPlaylistFromIndex:(int)index
{
    ...
    for (int i = index; i < [tempPlaylist count]; i++)
    {
...
-(void)removeAllObserversFromItems
{
    for (int i = 0; i < self.itemsForPlayer.count; i++)
    {
        [[NSNotificationCenter defaultCenter]removeObserver:self name:AVPlayerItemDidPlayToEndTimeNotification object:self.itemsForPlayer[i]];
    }
}
...
{
    BOOL didLoop = [self shouldLoopPlayer];
    if (!didLoop) // only if we haven't reached the end and already looped, we want to advance to the next item
    {
